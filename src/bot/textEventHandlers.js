...
async function actionDirectMessageEvent(ctx, next) {
  ...
  // If we don't know the user then we should tell them to use us prior to uploading
  if (await doesUserExist(message.author.id)) {
    return await next();
  } else {
    message.reply(
      `**Hi Friend!**\nYou're not in my database yet so please join a Voice Channel in the server you want to add an intro to!\nIf I'm not running in your server you'll need to ask an admin to add me. :smiley:`
    );
  }
}
...
async function vetDirectMessageAttachment(ctx) {
  ...
  if (message.attachments.array().length > 1) {
    ...
    p.warn(
      `DirectMessage from ${message.author.name} (${message.author.id}) does not contain an attachment ignore it`
    );
    return false;
  }
...
async function actionDirectMessageAttachment(ctx, next) {
  ...
  p.info(
    `Downloading attachment '${attachment.name}' sent by ${message.author.user} (${message.author.id})`
  );
