...
db.getIntroField = async function getIntroField({ checksum, field }) {
  return await db.getField({ dbCall: "getIntroField", query: { checksum, type: "sound" }, field });
};
...
db.insertType = async function insertType({ dbCall, type, document }) {
  p.debug(`Running INSERT from ${dbCall} -- type: ${type}, document: ${document}`);
  p.debug(document);
  return await db.raw.insert(Object.assign(document, { type }));
};

db.insertUser = async function insertUser({ userDoc }) {
  return await db.insertType({
    dbCall: "insertUser",
    type: "user",
    document: userDoc
  });
