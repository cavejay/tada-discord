...
db.bootReport = async function () {
  ...
  // User count and list
  let users = await db.raw.find({userID: {$exists: true}}, {name: 1, _id: 0, intro: 1})
...
db.getUsersWithIntros = async function (intros) {
  ...
  const res = await db.raw.find({$or: intros.map(introName => {
    return {
      intro: {
        type: 'file',
        key: introName
...
db.addSoundFile = async function (soundFile) {
  ...
  if (!res) {
    ...
    res = await db.raw.insert({
      soundFile: soundFile,
      name: soundFile.split('.').slice(0,-1).join('.')
    })
    return 1
  } else {
    return 0
...
db.getAllSounds = async function () {
