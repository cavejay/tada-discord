...
db.createUser = async function makeUser(username, userid, defaultUserConfig) {
  let newuser = { userID: userid, name: username, intro: defaultUserConfig };
...
db.addSoundFile = async function addSoundFile(soundFile) {
...
db.deprecateSound = async function deprecateSound(soundFile) {
  ...
  try {
    ...
    let removedElements = db.introCache.splice(db.introCache.indexOf(soundFile));
    p.info(`Removed ${removedElements} from the list of sound Files`);
  } catch (e) {
    p.error(e);
  }
