...
db.schema = {
  constructUser: function(uid, firstGuild, firstGuildIntro) {
    p.debug(`Creating user with - uid:${uid} - firstGuild:${firstGuild} - firstGuildIntro:${firstGuildIntro}`);
    let pres = {};
...
const dbUpgrades = {
  cleanDirtyUsers: require("./databaseUpdates/cleanDirtyUsers")
};
...
db.getObject = async function getObject({ dbCall, query }) {
  ...
  p.debug(`Returned GET-OBJECT from ${dbCall} -- result: ${JSON.stringify(res)}`);
  return res;
};
...
db.getUserObject = async function getUserObject({ uid }) {
  return await db.getObject({
    dbCall: "getUserObject",
    query: {
      type: "user",
      userid: uid
    }
...
db.getField = async function getField({ dbCall, query, field }) {
  ...
  p.debug(`Returned GET from ${dbCall} -- query: ${JSON.stringify(query)}, result: ${JSON.stringify(res[field])}`);
  return res[field];
};
...
db.update = async function update({ dbCall, query, updateDescription }) {
  p.debug(
    `Running UPDATE from ${dbCall} -- query: ${JSON.stringify(query)}, updateCommand: ${JSON.stringify(
      updateDescription
    )}`
  );
...
db.insertObject = async function insertObject({ dbCall, document }) {
  p.debug(`Running INSERT from ${dbCall} -- type: ${document.type}, document:`, document);
