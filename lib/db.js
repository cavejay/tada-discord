...
db.getSound = async function getSound(description) {
  p.info(`Attempting to fetch sound: ${description}`);

  const res = await db.raw.findOne(Object.assign({ type: "sound" }, description));
...
db.getSoundFileFromID = async function getSoundFileFromID(soundID) {
  if (!soundID) {
    throw `db.getSoundFileFromName -- soundName must be something was: ${soundID}`;
    return;
  }

  p.debug(`Getting file for sound: ${soundID}`);
  let res = await db.raw.findOne({ type: "sound", checksum: soundID }, { soundFile: 1, _id: 0 });
  p.debug(res);
...
db.deprecateSound = async function deprecateSound(soundID) {
  // Check it exists and fetch details
  const sound = await db.raw.findOne({ type: "sound", checksum: soundID });
  p.info(`Attempting to remove ${sound.name}'s entry in the database`);
  let res;
  try {
    res = await db.raw.remove({ checksum: soundID, type: "sound" });
    p.debug(res);
