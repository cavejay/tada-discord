...
module.exports = class RemoveIntro extends TadaCommand {
  constructor(client) {
    super(client, {
      ...
      examples: ["remove-intro <intro name>", "rm-intro <intro name> true", "rm-intro bbyshark true tada"],
      guildOnly: false
    });
  }
  ...
  async run(msg, args) {
    ...
    this.log.info(`User ${msg.author.username} is attempting to remove the '${introChoice}' intro.`);
    ...
    if (allSounds.includes(introChoice)) {
      ...
      let collected;
      try {
        collected = await confirmMsg.awaitReactions(reactionFilter, { time: 5000 });
      } catch (e) {
        this.log.error(e);
      }
      ...
      if (collected.first().count !== 2) {
        return await confirmMsg.reply(`Confirmation Emoji was not recieved. ${introChoice} will not be removed.`);
      } else {
        await confirmMsg.reply(`Confirmation recieved, removal continuing`);
      }

      // If it matches then update the database?
      this.log.info(`Updating user '${msg.author.username}''s registered intro to: ${introChoice}`);
      try {
      ...
      this.log.info(`'${introChoice}' is not a valid intro. Returning command here.`);
      return await msg.reply(
    ...
    return await msg.reply(`Update successful! The '${introChoice}' is no longer available`);
