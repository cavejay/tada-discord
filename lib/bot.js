...
// Handle what happens if someone joins a voice channel
...
module.exports = function bot ({db, config, soundManager}) {
    /**
    ...
    client.on("ready", () => {
        ...
        client.user.setPresence({ game: { name: config.playing }})
    ...
    client.on("message", async function (message) {
        ...
        if (message.content.startsWith(`${config.prefix}tada`)) {
            ...
            if (attachment.filename.slice(-4) == '.mp3') {
                p.info(`User's attachment '${attachment.filename}' was not an mp3 file - '${attachment.filename.slice(-4)}' != .mp3`)
                // message the player
                return
            }
