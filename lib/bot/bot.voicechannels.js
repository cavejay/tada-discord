...
function filterVoiceJoinEvent({ oldMember, newMember, config }) {
  ...
  // Joining debounce
  if (
    userIntroTracker[newMember.id] !== undefined &&
    userIntroTracker[newMember.id] + config.introDebounce > new Date() / 1
  ) {
    p.info("Time compare", userIntroTracker[newMember.id] + config.introDebounce, "<", new Date() / 1);
    p.info(`User ${newMember.displayName} did not have their intro played due to leaving < 10s ago`);
    return false;
  }
...
async function handleVoiceLeaveEvent({ oldMember, newMember, db, config }) {
  ...
  userIntroTracker[oldMember.id] = new Date() / 1;
