...
async function handleVoiceJoinEvent({ oldMember, newMember, opts }) {
  ...
  const tid = { uid: member.id, gid: member.guild.id };

  // Is this a new user?
  if (!(await member.client.userManager.checkUser(tid))) {
  ...
  let chosenIntro = await member.client.userManager.getUserIntro(tid);
  if (chosenIntro === "null") {
