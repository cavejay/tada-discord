...
async function playIntro({ member, intro, channel, db, config }) {
  ...
  if (intro.type === "yt") {
    ...
    p.info(
      `Bot has joined channel ${channel.name} to 'tada' ${member.username} with a snippet from a youtube video: '${
        chosenIntro.url
      }'`
    );
    ...
    if (channel.connection) {
      ...
      let connection = await channel.join();
      p.info(
        `Bot has joined channel ${channel.name} to 'tada' ${member.displayName} with preferred sound of '${intro.key}'`
      );
