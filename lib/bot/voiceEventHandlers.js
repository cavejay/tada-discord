...
async function filterTadaJoinEvent_guild(ctx, next) {
  ...
  const disabledChannels = (await di.getGuildChannelConfig(ctx.gid)).map(
    (c) => {
...
async function filterTadaJoinEvent_user(ctx, next) {
  ...
  ctx.targetIntroID = await di.getUserIntro(ctx.gid, ctx.uid);
  if (ctx.targetIntroID == undefined) {
    // setup the new user
...
async function actionTadaJoinEvent(ctx, next) {
  ...
  // Does the user have custom intro for this guild yet?
  const userIntroForGuild = await di.getUserIntro(ctx.gid, ctx.uid);

  const introId = !userIntroForGuild
    ? await di.getGuildProp(ctx.gid, "defaultIntro")
    : userIntroForGuild;

  await playTadaNoise(introId, ctx.newState.channel, introTime);
