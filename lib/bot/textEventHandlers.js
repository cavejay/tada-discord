...
async function actionDirectMessageAttachment(ctx, next) {
  ...
  await getGuildAssignmentInfo(message.author, ctx.client);
  ...
  await getGuildAssignmentInfo(message.author.id, ctx.client);

  // allow the intro to be used by the chosen reactions
}

async function getGuildAssignmentInfo(discordUser, client) {
  ...
  const userGuilds = await Promise.all(
    guildids.map(async (gid) => (await client.guilds.fetch(gid)).name)
  );
  ...
  const allEmotes = alpha;

  p.debug({ msg: "reaction things", alpha, allEmotes });
  ...
  const guildsSelected = await collectUserReactionInput(
    questionMessage,
