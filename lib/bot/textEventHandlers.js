...
async function actionDirectMessageAttachment(ctx, next) {
  ...
  // Get which guilds this user and we are part of?
  const glist = await getUsersGuilds(message.author.id, ctx.client);
  directMessageUser(message.client, message.author.id, `${glist.join(" - ")}`);
}
