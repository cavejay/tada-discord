...
async function getIntrosUploadedByUser(uid) {
  p.debug({
    msg: `Getting all intros uploaded by user '${uid}'`,
    action: "getIntrosUploadedByUser",
    uid,
  });
  return await global.db.run((r) =>
    r.table("intros").filter({ uploader: uid }).pluck("name").coerceTo("array")
  );
}
...
async function getGuildIntroConfig(gid, id) {
  p.debug({
    msg: `Getting any custom intro config for '${id}' in guild '${gid}'`,
    action: "getGuildIntroConfig",
    gid,
    id,
  });
  return await global.db.run((r) =>
...
async function setGuildIntroConfig(gid, id, obj) {
  p.debug({
    msg: `Setting custom intro config for '${id}' to '${obj}' in guild '${gid}'`,
    action: "setGuildIntroConfig",
    gid,
    id,
    obj,
  });

  let newIntroConfig = { introSpecificConfig: {} };
  newIntroConfig.introSpecificConfig[id] = obj;

  return await global.db.run((r) =>
    r.db(global.db.getDBName()).table("guilds").get(gid).update(newIntroConfig)
  );
}
...
async function createUser(uid, gid) {
  p.debug({
    msg: `Creating new user '${uid}' with an initial guild '${gid}'`,
    action: "createUser",
    gid,
    uid,
  });
  ...
  const newUser = {
    userid: uid,
    guilds: guildsObj,
  }; // todo streamline this with r.object
...
async function doesUserExist(uid) {
  p.debug({
    msg: `Checking that user '${uid}' is known by us`,
...
async function ensureUserGuildConnection(uid, gid) {
  p.debug({
    msg: `Ensuring that user '${uid}' is linked to guild '${gid}'`,
    action: "ensureUserGuildConnection",
    gid,
    uid,
  });
