  ...
  return await global.db.run((r) =>
    r.branch(
      r
        ...
        .keys()
        .contains(cid),
      r.db("TadaDB_v1").table("guilds").get(gid)("channelConfig")(cid),
      r.db("TadaDB_v1").table("guilds").get(gid)("defaultChannelConfig")
    )
  );
}
  ...
  try {
    const retVal = await global.db.run((r) =>
      r
        ...
        .table("users")
        .get(uid)("guilds")(gid)
        .default(r.db("TadaDB_v1").table("guilds").get(gid)("defaultIntro"))
    );
