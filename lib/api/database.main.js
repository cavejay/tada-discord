...
class DBHandler {
  ...
  async init() {
    ...
    let res = await r
      ...
      .update(
        {
          lastBoot: startDate,
          historicalBootups: r.table("meta").get(0)("historicalBootups").append(r.table("meta").get(0)("lastBoot")),
        },
        { nonAtomic: true }
      )
      .run(this.conn);
  ...
  async setupResources() {
    ...
    const requiredTables = [{ name: "guilds", id: "guildid" }, { name: "users" }, { name: "meta" }, { name: "intros" }];
    await Promise.all(
      requiredTables.map((table) => {
        if (!tableList.includes(table.name)) {
          p.warn(`Database '${this.databaseName}' missing table '${table.name}' - proceeding to create`);
    ...
    if (!metaTableData) {
      ...
      try {
        ...
        p.error({ msg: "Failed to create the default metaData Table", error: e });
      }
    }
  }
