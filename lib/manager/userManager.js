...
um.getUserIntro = async function getUserIntro({ gid, uid, readable }) {
  p.debug(`Finding intro listed for ${uid} in ${gid}`);
  const retval = await userCache.getOrSet(`${uid}-${gid}`, async () => {
  ...
  if (!readable) {
    ...
    return await um.sm.getNameFromChecksum({ checksum: retval, gid });
  }
};

um.setUserIntro = async function setUserIntro({ gid, uid, name }) {
  ...
  let userPresence = await um.db.getUserField({ uid, field: "presence" });
  userPresence[gid] = soundCS;

  const res = await um.db.updateUserField({ uid, field: "presence", value: userPresence });

  // Only then update the cache
  userCache.set(`${uid}-${gid}`, soundCS);
};
