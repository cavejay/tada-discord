...
var tmpfolder = "";
const FileCache = new cacher("FileCache"); // introname -> filename
...
fm.init = async function({ db, config }) {
  ...
  if (!fs.existsSync(fm.soundFolder)) {
    p.error(
      `The folder used to store sounds files no longer exists. Please restore '${
        fm.soundFolder
      }' and the original, included intros`
    );
    process.exit();
  }
...
/**
 * Download the URL and save to the configured 'tmp' folder with filename filename-date-checksum.mp3"
 */
fm.downloadFile = async function(url) {
  const filename = url.split("/").slice(-1)[0];
  const destination = path.join(fm.soundFolder, "tmp", filename);
  ...
  let res = await httpsgetP(url);
  p.info("finished creating the pipe, now waiting till it's done");
  let pp = await promisePipe(res, file);
 ...
 */
fm.checksumFile = async function checksumFile(path) {
  return new Promise((resolve, reject) =>
    fs
...
fm.moveFileToSoundsFolder = async function(filename) {
  try {
    await moveFile(path.join(tmpfolder, filename), path.join(fm.soundFolder, filename));
    return true;
  } catch (e) {
    p.error("Unable to move file", e);
    throw e;
  }
  return false;
};
 ...
 */
fm.deleteFile = async function({ filename }) {
  try {
